...
在 Claude Code 生态中，MCP server 主要有三种形态：
...
- **Python 3.14+**
- **[uv](https://docs.astral.sh/uv/)** — 现代 Python 包管理器（比 pip 快 10-100 倍）
...
name = "twitter-mcp"
version = "0.1.0"
description = "Twitter MCP server powered by twikit"
readme = "README.md"
...
[project.scripts]
twitter-mcp = "twitter_mcp.server:main"
```

依赖说明：
- **`mcp[cli]`** — Anthropic 官方 MCP Python SDK，包含 `FastMCP` 高层封装
- **`twikit @ git+...`** — 从 GitHub PR#412 分支安装 twikit（修复了主分支的 bug）

> **关于 PR#412：** twikit 主分支的 `get_tweet_by_id()` 有 entries 解析 bug。
> PR#412 包含修复，同时还合并了 PR#410 和 PR#411 的改进。
> 如果将来这些 PR 合并到主分支，可以简化为 `"twikit"` 即可。
...
from twikit import Client                  # Twitter 逆向工程库
...
这会：
1. 创建 `.venv/` 虚拟环境
2. 从 GitHub PR#412 克隆并安装 twikit
3. 安装 `mcp[cli]` 和所有传递依赖
4. 生成 `uv.lock` 锁定文件
...
| `--` | 分隔符，之后是启动命令 |
| `uv run --directory ... python -m twitter_mcp.server` | 启动命令 |
...
### 报错 "twikit entries parsing error"？

确保用的是 PR#412 分支。检查 `pyproject.toml` 中的依赖：
```toml
twikit @ git+https://github.com/d60/twikit.git@refs/pull/412/head
```
...
如果你想在另一台电脑（比如公司电脑、服务器、VPS）上也能用这个 Twitter MCP，按以下步骤操作。
...
如果你已经把 twitter-mcp 推到了 GitHub/GitLab：

```bash
mkdir -p ~/mcp-servers
cd ~/mcp-servers
git clone https://github.com/你的用户名/twitter-mcp.git
```

**方式 B：直接复制目录**

如果没有 git 仓库，用 rsync 或 scp 从源机器复制：

```bash
# 在源机器上执行（替换 user@target 为目标机器的 SSH 地址）
rsync -avz --exclude '.venv' --exclude '__pycache__' \
  ~/mcp-servers/twitter-mcp/ user@target:~/mcp-servers/twitter-mcp/
```

> 排除 `.venv/` 是因为虚拟环境不可移植（路径和 Python 版本可能不同），目标机器上会重新创建。

**方式 C：手动创建**

按本文档 [从零搭建](#从零搭建) 章节，在目标机器上从头建。只需要复制 `pyproject.toml` 和 `twitter_mcp/server.py` 两个文件。
...
```bash
uv run python -c "from twitter_mcp.server import mcp; print('OK')"
```

#### Step 3: 配置 Twitter Cookies

你需要在目标机器上放置 cookies 文件。有两种情况：

**同一个 Twitter 账号：** 直接复制 cookies 文件
...
# 从源机器复制（在源机器上执行）
scp ~/.config/twitter-mcp/cookies.json user@target:~/.config/twitter-mcp/

# 在目标机器上限制权限
chmod 600 ~/.config/twitter-mcp/cookies.json
```

**不同的 Twitter 账号：** 在目标机器的浏览器上登录对应的 Twitter 账号，按 [获取 Twitter Cookies](#获取-twitter-cookies) 章节提取 cookies。
...
{
  "ct0": "目标机器浏览器里的ct0",
  "auth_token": "目标机器浏览器里的auth_token"
}
EOF
chmod 600 ~/.config/twitter-mcp/cookies.json
```

> **注意：** cookies 和浏览器登录状态绑定。如果你在源机器的浏览器上退出 Twitter 登录，复制过去的 cookies 也会失效。建议在目标机器上用自己的浏览器重新提取。
...
然后说："搜一下关于 AI 的推文" 或 "看看我的时间线"。

### 一键部署脚本

如果你经常需要在新机器上部署，可以保存这个脚本：

```bash
#!/bin/bash
# deploy-twitter-mcp.sh
# 用法: ./deploy-twitter-mcp.sh
# 前置: 已安装 python3.14+, uv, claude code, git
...
if [ -d "twitter-mcp" ]; then
    echo "twitter-mcp 已存在，拉取最新代码..."
    cd twitter-mcp && git pull
else
    # 替换为你的仓库地址
    git clone https://github.com/你的用户名/twitter-mcp.git
    cd twitter-mcp
...
echo "=== Step 2: 安装依赖 ==="
uv sync
uv run python -c "from twitter_mcp.server import mcp; print('依赖安装 OK')"

echo "=== Step 3: 配置 Cookies ==="
mkdir -p ~/.config/twitter-mcp
if [ ! -f ~/.config/twitter-mcp/cookies.json ]; then
    echo "请创建 cookies 文件："
    echo "  ~/.config/twitter-mcp/cookies.json"
    echo "  格式: {\"ct0\": \"...\", \"auth_token\": \"...\"}"
    echo "  从浏览器 DevTools > Application > Cookies > x.com 获取"
    echo ""
    read -p "已创建？按 Enter 继续..."
fi
chmod 600 ~/.config/twitter-mcp/cookies.json

echo "=== Step 4: 注册到 Claude Code ==="
# 先删除旧的（如果有）
...
echo ""
echo "部署完成！重启 Claude Code (/exit 然后 claude) 即可使用。"
```

使用方法：

```bash
chmod +x deploy-twitter-mcp.sh
./deploy-twitter-mcp.sh
```
...
□ uv --version               → 已安装
□ claude --version            → 已安装
□ ls ~/mcp-servers/twitter-mcp/twitter_mcp/server.py  → 文件存在
□ cat ~/.config/twitter-mcp/cookies.json               → ct0 和 auth_token 存在
□ stat -c %a ~/.config/twitter-mcp/cookies.json        → 600（仅本用户可读写）
□ cd ~/mcp-servers/twitter-mcp && uv run python -c \
    "from twitter_mcp.server import mcp; print('OK')"  → OK
...
如果你修改了 server 代码（比如加了新工具），需要同步到其他机器：
...
# 目标机器上重装依赖
ssh user@target "cd ~/mcp-servers/twitter-mcp && uv sync"
```

> 更新代码后不需要重新 `claude mcp add`，只需要重启 Claude Code（`/exit` 再进入）。
