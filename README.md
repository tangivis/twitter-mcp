...
> **X Articles note**: long-form posts at `https://x.com/i/article/<id>` live in a different ID namespace than tweets. `get_tweet` refuses them with a clear error pointing to `get_article`. `get_article_preview` works without auth via the public syndication endpoint. `get_article` calls the `ArticleEntityResultByRestId` GraphQL persisted query — its queryId is hardcoded just like the 80+ other twikit endpoints; if X rotates the hash, refresh it from the public `bundle.TwitterArticles.*.js` chunk on `abs.twimg.com` (no auth needed for discovery).
...
> **X Articles 说明**:`https://x.com/i/article/<id>` 这种长文与普通推文是两个 ID 命名空间。`get_tweet` 遇到 article URL 会直接拒绝并提示用 `get_article`。`get_article_preview` 走公共 syndication 端点,不需要登录;`get_article` 调 `ArticleEntityResultByRestId` GraphQL 持久化查询,queryId 跟其他 80+ 端点一样硬编码 — 如果 X 改了 hash,从公开的 `bundle.TwitterArticles.*.js` chunk 里 grep 出新 hash 即可,无需登录态。
...
> **X Articles について**:`https://x.com/i/article/<id>` の長文記事はツイートとは別の ID 名前空間に属します。`get_tweet` は article URL を渡されると拒否し、`get_article` を案内します。`get_article_preview` は公開 syndication エンドポイント経由で認証不要。`get_article` は `ArticleEntityResultByRestId` GraphQL persisted query を呼び、queryId は他の 80+ エンドポイント同様ハードコードされています — X がハッシュを変更した場合は、公開されている `bundle.TwitterArticles.*.js` チャンクから新しいハッシュを取得できます(認証不要)。
