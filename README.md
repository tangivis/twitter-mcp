...
| `get_user_info` | Get a user's profile metadata (id, name, bio, follower counts, ...) |
...
| `like_tweet` | 点赞 | "点赞这条推" |
| `retweet` | 转推 | "转推这个" |
| `get_user_info` | 查看用户资料（id、名字、简介、粉丝数等） | "查一下 @xxx 的资料" |
...
| `get_user_info` | ユーザープロフィール取得（id・名前・bio・フォロワー数など） | 「@xxx のプロフィールを見せて」 |
