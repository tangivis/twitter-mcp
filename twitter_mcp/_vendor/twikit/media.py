...
class AnimatedGif(Media):
    ...
    def streams(self) -> list:
        return [
            Stream(self._client, stream_data)
            for stream_data in self.video_info.get("variants")
        ]
...
class Video(Media):
    ...
    def streams(self) -> list[Stream]:
        video_streams = filter(
            lambda x: x["content_type"].startswith("video"), self._streams
        )
    ...
    async def _get_playlist(self) -> M3U8 | None:
        ...
        m3u8_stream = next(
            filter(
                lambda x: x["content_type"] == "application/x-mpegURL", self._streams
            ),
            None,
        )
        if not m3u8_stream:
    ...
    async def _get_subtitles_playlist(self) -> M3U8 | None:
        ...
        subtitles_media = next(
            filter(lambda x: x.type == "SUBTITLES", playlist.media), None
        )
    ...
    async def get_subtitles(self) -> webvtt.WebVTT | None:
        ...
        if not subtitles_playlist:
            return None
        response, _ = await self._client.get(
            self._base_url + subtitles_playlist.segments[0].uri
        )
