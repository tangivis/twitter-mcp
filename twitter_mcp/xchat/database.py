"""Read XChat plaintext from X's local SQLite store (issue #118).

X's web client decrypts XChat conversations locally and writes the result
to a SQLite database inside Chromium's origin-private filesystem. This
module opens that file with SQLite's read-only + immutable URI flags and
projects only conversation metadata and the `plain_text` column.

The browser stays responsible for authentication, E2EE decryption, and
sync. This reader is structurally incapable of sending a message or
mutating read state: every statement is a SELECT, and the key-material
table is only ever counted, never read for content.

Derived from @DJNgoma's implementation in PR #107, narrowed to the
...
# Validated up front so a wrong file fails with a clear message instead of
# an obscure "no such table" at query time. X can change this schema
# without notice — see the fragility note in #118.
...
def _timestamp(value: Any) -> str | None:
    """X stores epoch milliseconds. Return ISO-8601 Z, or None if unusable."""
    try:
        return (
...
def _entry_text(text: Any, has_attachment: Any, types: Any) -> str:
    """Never return an empty string.

    An agent that sees "" cannot distinguish "they sent nothing" from
    "this message carried only an image". Both get an explicit
    placeholder instead.
    """
    ...
    if clean:
        return clean
    if has_attachment:
        kind = str(types or "").strip()
        return f"[{kind} attachment]" if kind else "[attachment]"
    return "[message]"


class XChatDatabase:
    """A strictly read-only view over X's locally decrypted XChat store."""
    ...
    def _connect(self) -> Iterator[sqlite3.Connection]:
        if not self.path.is_file():
            raise XChatUnavailable(
                f"No XChat database at {self.path}. Open XChat in your browser, "
                "unlock it, and let it finish syncing."
            )
        # immutable=1 additionally promises SQLite the file will not change
        # under us, so it takes no locks and cannot create -wal/-shm
        # sidecars next to the user's browser profile.
    ...
    def _validate(con: sqlite3.Connection) -> None:
        ...
        if missing:
            raise XChatExtractionFailed(
                "That file is missing required XChat tables: " + ", ".join(missing)
            )

    @staticmethod
    def _owner_id(con: sqlite3.Connection) -> str | None:
        """Infer the local account id from the most frequent owner sender."""
        row = con.execute(
            """
            SELECT sender_id
            FROM dm_entry
    ...
    def _screen_names(con: sqlite3.Connection, user_ids: set[str]) -> dict[str, str]:
        # Every id is forced through int() before it reaches the query, so
        # the generated placeholder list can never carry caller data.
    ...
    @staticmethod
    def _direct_peer_id(conversation_id: str, owner_id: str | None) -> str | None:
        """The other participant's id, for two-person conversations only.

        Group conversation ids are prefixed `g`; they have no single peer.
        """
    ...
    def status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "detail": "XChat messages are readable from the browser's local decrypted store.",
        }
    ...
    def list_conversations(
        ...
        with self._connect() as con:
            ...
            peer_ids = {
                peer
                for row in rows
                if (peer := self._direct_peer_id(str(row["conversation_id"]), owner_id))
            }
            names = self._screen_names(con, peer_ids)

        conversations: list[dict[str, Any]] = []
        for row in rows:
            conversation_id = str(row["conversation_id"])
            peer_id = self._direct_peer_id(conversation_id, owner_id)
            screen_name = names.get(peer_id or "")
            ...
            conversations.append(
                {
                    ...
                    "name": title or screen_name or peer_id or conversation_id,
    ...
    def get_history(
        ...
        limit = _clamp(limit)
        with self._connect() as con:
            # Inner query takes the newest `limit` rows, outer flips them
        ...
        if not rows:
            raise XChatExtractionFailed(
                ...
                "Check the id with xchat_list_conversations."
            )
        return [
            {
                "text": _entry_text(
    ...
    def doctor(self) -> dict[str, Any]:
        """Raw counts for diagnosing an unexpected result. No message content."""
        with self._connect() as con:
            table_count = int(
                con.execute(
