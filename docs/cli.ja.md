...
実際のターミナルでは、作者名は太字シアン、タイムスタンプは薄色、`❤` は赤、`🔁` は緑、URL は OSC 8 エスケープで包まれます — iTerm2 / kitty / WezTerm / Windows Terminal / gnome-terminal ≥ 3.36 で cmd-クリックすればブラウザで開きます。emoji と CJK の行は**列幅が正しい**(Rich の cell-aware 計測でパディング — 0.1.23 にあった右ボーダーのずれは解消)。
