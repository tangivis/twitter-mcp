...
| 0.1.28 | `_vendor/twikit/list.py` + `_vendor/twikit/client/client.py` | List + 列表相关 client 方法的全面防御化(issue #76 part 1)。三处修复:(a) `List.__init__` 把 11 个 `data["X"]` 改为 `.get(X, default)`,X 漏发任何字段不再 KeyError(其中 `created_at` 是 live-smoke 实际撞到的);(b) `Client._get_list_users` 把 `find_dict(...)[0]` 改成防 empty,burner 看不到 members/subscribers 时返回空 Result 而非 IndexError;(c) `Client.get_list_tweets` 在 `entries=[]` 时返回空 Result(老代码 `items[-1]` 直挂)。配套测试 `tests/test_list_parsing.py`(18 个)+ live-smoke 的 `T_LIST` 不再包含 `T_DRIFT` 兜底,真出现这类 IndexError/KeyError 现在会真红 CI。|
| 0.1.29 | `_vendor/twikit/community.py` + `_vendor/twikit/client/client.py` + `twitter_mcp/server.py` | Community + community 相关 client 方法 + article preview 全面防御化(issue #76 parts 2+3)。修复:(a) `Community.__init__` + `CommunityMember.__init__` 把所有 `data["X"]` 和 `legacy["X"]` 改 `.get(default)`,只保留 `rest_id` strict;facepile 的多层 `i["result"]["legacy"]["X"]` 用 `.get()` 链 + skip 不全的 entry;(b) `Client._get_community_users` 把 `find_dict(...)[0]` 改成防 empty;(c) `Client.get_community_tweets` 和 `Client.search_community_tweet` 在 `entries`/`items` 为空时返回空 Result(老代码 `items[-1]/items[-2]` 直挂);(d) `server.py::get_article_preview` 把 syndication 端点 404 转成干净的 `ToolError`,不再泄露 `HTTPStatusError` 堆栈。配套测试 `tests/test_community_parsing.py`(30 个)+ `tests/test_articles.py` 加 1 个。live-smoke 的 `T_COMM` / `T_SEARCH` / `T_ARTICLE_PREVIEW` 全拆掉了 `T_DRIFT` 兜底。**至此 issue #76 整体完成,`T_DRIFT` 在 live-smoke.yml 里成为死代码**(可选清理)。|
| 0.1.37 | `_vendor/twikit/client/client.py` + `twitter_mcp/server.py` | `Client.get_dm_history` 防御化(issue #104)。X 的 1:1 DM timeline 会塞 `trust_conversation` 等非 message entry;上游 `item["message"]["message_data"]` 直挂 `KeyError: 'message'`。改成 skip 非 message、把 system event 挂到 `Result.timeline_events`、仅 system event 时返回空 Result(不再 `messages[-1]` IndexError)。group path 同样加固 empty-list。`server.get_dm_history` 把 `timeline_events` 透出,并在出现 system event 时加 `warnings`(legacy DM API 拿不到 X Chat / E2EE 密文,UI 里更多消息时历史不完整)。配套 `tests/test_vendor.py` + `tests/test_tools.py`。|
| 0.1.42 | `_vendor/twikit/client/client.py` | `Client._get_tweet_engagements` 防御化(issue #37,2026-08-17 live-smoke 撞到)。两处修复:(a) 被封禁/注销的转推者 X 返回 `__typename: UserUnavailable`,该 result 没有 `rest_id`,而 `User.__init__` 硬取这个 key —— 一个死账号就让整次 `get_retweeters` 挂 `KeyError: 'rest_id'`;现在跳过没有 `rest_id` 的 result。(b) 同一函数里 `items[-1]["content"]["value"]` 假设最后两条 entry 一定是 cursor,X 限流返回不含 cursor 时会在紧邻的一行 KeyError;现在取不到就是 `None`。`get_favoriters` 走同一函数,一并受益。打了 `# twitter-mcp patch (issue #37)` 标记 + `tests/test_vendor.py` 5 个配套测试(含 source marker),下次 vendor 刷新别漏。|
